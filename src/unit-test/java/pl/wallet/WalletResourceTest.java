...
import pl.tool.WalletRandomTool;
import pl.user.User;
...
class WalletResourceTest {
   ...
   void init() {
      ...
      walletService = new WalletService(walletRepository, userProvider, transactionService, friendShipService);
   ...
   void addWalletReturnWalletDtoWhenAddingWalletFinishSuccessful() {
      ...
      WalletDto expectedWalletDto = WalletDto.builder()
         ...
         .users(new HashSet<>(Set.of(UserDto.builder().email(userDto.getEmail()).build())))
   ...
   void editWalletReturnEditedWalletWhenNewWallet() {
      ...
      // then
      WalletDto expectedWalletDto = getWalletDtoBuilder(walletWithUpdatedFields).id(wallet.getId()).owner(UserDto.builder().email(user.getEmail()).build()).users(Set.of(UserDto.builder().email(user.getEmail()).build())).balance(wallet.getBalance()).build();
   ...
   void shareWalletWithFriendAddingFriendToWalletUsersWhenWalletIsOnCallUserOwn() {
      ...
      User user = User.builder().email(userDto.getEmail()).build();
      User friend = User.builder().email(friendDto.getEmail()).build();
      Wallet wallet = WalletRandomTool.randomWallet(userDto);
      wallet.setId(1L);
      wallet.setOwner(user);
      wallet.addUser(user);
      ...
      when(friendShipService.isFriends(any(), any())).thenReturn(true);
      ...
      // then
      WalletDto expectedWalletDto = getWalletDtoBuilder(WalletMapper.toDto(wallet)).owner(UserDto.builder().email(user.getEmail()).build()).users(Set.of(UserDto.builder().email(userDto.getEmail()).build(), UserDto.builder().email(friendDto.getEmail()).build())).build();
      assertEquals(HttpStatus.OK, walletDtoResponseEntity.getStatusCode());
      assertEquals(expectedWalletDto, walletDtoResponseEntity.getBody());
   }
   ...
   void changeWalletOwnerReturnWalletDtoWithNewOwnerWhenUserIsOwnerAnd() {
      ...
      User user = User.builder().email(userDto.getEmail()).build();
      User newOwner = User.builder().email(newOwnerDto.getEmail()).build();
      Wallet wallet = WalletRandomTool.randomWallet(userDto);
      wallet.setId(1L);
      wallet.setOwner(user);
      wallet.addUser(user);
      ...
      // then
      WalletDto expectedWalletDto = getWalletDtoBuilder(WalletMapper.toDto(wallet)).owner(UserDto.builder().email(newOwnerDto.getEmail()).build()).users(Set.of(UserDto.builder().email(userDto.getEmail()).build(), UserDto.builder().email(newOwnerDto.getEmail()).build())).build();
      assertEquals(HttpStatus.OK, walletDtoResponseEntity.getStatusCode());
      assertEquals(expectedWalletDto, walletDtoResponseEntity.getBody());
   }

   @Test
   void removeUserFromWalletReturnWalleDtoWhenRemovingUserIsNotOwner() {
      // given
      UserDto userDto = UserRandomTool.randomUserDto();
      UserDto removingUserDto = UserRandomTool.randomUserDto();
      User user = User.builder().email(userDto.getEmail()).build();
      User removingUser = User.builder().email(removingUserDto.getEmail()).build();
      Wallet wallet = WalletRandomTool.randomWallet(userDto);
      wallet.setId(1L);
      wallet.setOwner(user);
      wallet.addUser(user);
      ...
      // then
      WalletDto expectedWalletDto = getWalletDtoBuilder(WalletMapper.toDto(wallet)).owner(UserDto.builder().email(userDto.getEmail()).build()).users(Set.of(UserDto.builder().email(userDto.getEmail()).build())).build();
      assertEquals(HttpStatus.OK, walletDtoResponseEntity.getStatusCode());
      assertEquals(expectedWalletDto, walletDtoResponseEntity.getBody());
   }
